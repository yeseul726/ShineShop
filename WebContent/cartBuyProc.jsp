...
<%
	...
	sql = "SELECT item_id FROM ss_cart WHERE user_id='" + (String)application.getAttribute("user_id") + "'";
	...
	while(rs.next()) {
		...
		PreparedStatement cnt_ps = conn.prepareStatement(sql);
		
		sql = "SELECT item_buyCnt FROM ss_item WHERE item_id='" + rs.getString("item_id") + "'";
		
		//out.println(sql);
		
		cnt_rs = cnt_stmt.executeQuery(sql);
		
		cnt_rs.next();
		
		upCnt = cnt_rs.getInt("item_buyCnt") + 1;
		
		sql = "UPDATE ss_item SET item_buyCnt='" + upCnt + "' WHERE item_id='" + rs.getString("item_id") + "'";
		
		//out.println(sql);
		
		cnt_ps = conn.prepareStatement(sql);
		
		//String sql = "UPDATE " + tableName + " SET " + recom + "'"
		//UPDATE 테이블명 SET 필드명='변경할값' WHERE 필드명=해당값;
		
		cnt_ps.executeUpdate();
	}
	...
	sql = "DELETE FROM ss_cart WHERE user_id='" + (String)application.getAttribute("user_id") + "'";
	PreparedStatement ps = conn.prepareStatement(sql);
	ps.executeUpdate();
